...
def read_macint32_file(file_path):
    ...
    with open(file_path+'A_decimal.txt', 'r') as file:
        ...
        for line in file:
            value = line.strip()
            if value:
                a.append(int(value))  # Append the value to the list
    with open(file_path+'B_decimal.txt', 'r') as file:
        next(file)  # Skip header if present
        for line in file:
            value = line.strip()
            if value:
                b.append(int(value))  # Append the value to the list
    with open(file_path+'C_decimal.txt', 'r') as file:
        next(file)  # Skip header if present
        for line in file:
            value = line.strip()
            if value:
                c.append(int(value))  # Append the value to the list
...
def read_macfp32_file(file_path):
    ...
    with open(file_path+'A_decimal.txt', 'r') as file:
        ...
        for line in file:
            value = line.strip()
            if value:
                a.append(float(value))  # Append the value to the list
    with open(file_path+'B_decimal.txt', 'r') as file:
        next(file)  # Skip header if present
        for line in file:
            value = line.strip()
            if value:
                b.append(float(value))  # Append the value to the list
    with open(file_path+'C_decimal.txt', 'r') as file:
        next(file)  # Skip header if present
        for line in file:
            value = line.strip()
            if value:
                c.append(float(value))  # Append the value to the list
    return list(zip(a,b,c))
...
def float_to_ieee754(value):
    """Convert a float to its IEEE 754 representation as a 32-bit integer."""
...
async def test_mac(dut):
    ...
    input_values = read_macint32_file('/home/karan/Downloads/CS6230_project1_v2/test_cases/int8 MAC/')
    ...
    # Read input values from the file
    input_values_fp = read_macfp32_file('/home/karan/Downloads/CS6230_project1_v2/test_cases/bf16 MAC/')

    for a, b, c in input_values_fp:

        a_encode = float_to_ieee754(a)& 0xFFFF
        b_encode = float_to_ieee754(b)& 0xFFFF
        c_encode = float_to_ieee754(c)
        
        # Set value of A and wait for RDY signal
        dut.get_A_value.value = a_encode
        dut.EN_get_A.value = 1  # Enable A input
        await RisingEdge(dut.CLK)
        assert dut.RDY_get_A.value == 1, "RDY_get_A not asserted"
        dut.EN_get_A.value = 0  # Disable after setting
        ...
        # Set value of B and wait for RDY signal
        dut.get_B_value.value = b_encode
        dut.EN_get_B.value = 1  # Enable B input
        await RisingEdge(dut.CLK)
        assert dut.RDY_get_B.value == 1, "RDY_get_B not asserted"
        dut.EN_get_B.value = 0  # Disable after setting
        ...
        # Set value of C and wait for RDY signal
        dut.get_C_value.value = c_encode
        dut.EN_get_C.value = 1  # Enable C input
        await RisingEdge(dut.CLK)
        assert dut.RDY_get_C.value == 1, "RDY_get_C not asserted"
        dut.EN_get_C.value = 0  # Disable after setting
...
#async def test_mac_int(dut, input_file='', select=0):
...
#        await RisingEdge(dut.CLK)
#        assert dut.RDY_get_MAC.value == 1, "RDY_get_MAC not asserted"
        #dut._log(f'{int(dut.get_MAC.value}')
